...
namespace PersonalMethod {
	...
	public:
		...
		CMD5(const std::string& str);                          //输入字符串的构造函数
		CMD5(std::ifstream& in);                               //输入流的构造函数
		...
		void update(const std::string& str);                   //添加字符串
		void update(std::ifstream& in);                        //添加流
		...
		std::string toString();                                //计算MD5码,并返回其对应的字符串
		void reset();                                     //重置
	private:
