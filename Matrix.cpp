...
// 返回值：bool 型，转换是否成功
//////////////////////////////////////////////////////////////////////
bool CMatrix::FromString(string s, const string& sDelim /*= " "*/, bool bLineBreak /*= true*/)
{
	if (s.empty())
		return false;
	...
	if (bLineBreak)
	{
		...
		vector<string> ListRow;
		string sRow;
		while (tk.Next(sRow))
		{
			CTokenizer::TrimLeftSpace(sRow);
			CTokenizer::TrimRightSpace(sRow);
			if (sRow.empty())
				break;

			ListRow.push_back(sRow);
		}

		// 行数
		m_nNumRows = ListRow.size();
		...
		for (int i = 0; i<m_nNumRows; i++)
		{
			...
			while (tkRow.Next(sElement))
			{
				CTokenizer::TrimLeftSpace(sElement);
				CTokenizer::TrimRightSpace(sElement);
				double v = atof(sElement.c_str());
				SetElement(i, j++, v);
			}
		}
	...
	CTokenizer::TrimLeftSpace(sElement);
	CTokenizer::TrimRightSpace(sElement);
	m_nNumRows = atoi(sElement.c_str());

	// 列数
	tk.Next(sElement);
	CTokenizer::TrimLeftSpace(sElement);
	CTokenizer::TrimRightSpace(sElement);
	m_nNumColumns = atoi(sElement.c_str());
...
//
// 参数：
// 1. const string& sDelim - 数字之间的分隔符，默认为空格
// 2 bool bLineBreak - 行与行之间是否有回车换行符，默认为真(有换行符)
//
// 返回值：string 型，转换得到的字符串
//////////////////////////////////////////////////////////////////////
string CMatrix::ToString(const string& sDelim /*= " "*/, bool bLineBreak /*= true*/) const
{
	string s = "";
...
// 返回值：string 型，转换得到的字符串
//////////////////////////////////////////////////////////////////////
string CMatrix::RowToString(int nRow, const string& sDelim /*= " "*/) const
{
	string s = "";
...
// 返回值：string 型，转换得到的字符串
//////////////////////////////////////////////////////////////////////
string CMatrix::ColToString(int nCol, const string& sDelim /*= " "*/) const
{
	string s = "";
...
//	ASSERT(m_pData);							// bad pointer error
	if (m_pData && nCol >= 0 && nCol < m_nNumColumns && nRow >= 0 && nRow < m_nNumRows)
...
// 2. int nCols - 指定的矩阵列数
//////////////////////////////////////////////////////////////////////
CComplexMatrix::CComplexMatrix(int nRows, int nCols)
{
	m_nNumRows = nRows;
	m_nNumColumns = nCols;
	m_pData = NULL;
...
// 3. double value[] - 一维数组，长度为nRows*nCols，存储矩阵各元素的值
//////////////////////////////////////////////////////////////////////
CComplexMatrix::CComplexMatrix(int nRows, int nCols, CComplex value[])
{
	m_nNumRows = nRows;
	m_nNumColumns = nCols;
	m_pData = NULL;
...
// 1. int nSize - 方阵行列数
//////////////////////////////////////////////////////////////////////
CComplexMatrix::CComplexMatrix(int nSize)
{
	m_nNumRows = nSize;
	m_nNumColumns = nSize;
	m_pData = NULL;
...
// 2. double value[] - 一维数组，长度为nRows*nRows，存储方阵各元素的值
//////////////////////////////////////////////////////////////////////
CComplexMatrix::CComplexMatrix(int nSize, CComplex value[])
{
	m_nNumRows = nSize;
	m_nNumColumns = nSize;
	m_pData = NULL;
...
// 1. const CMatrix& other - 源矩阵
//////////////////////////////////////////////////////////////////////
CComplexMatrix::CComplexMatrix(const CComplexMatrix& other)
{
	m_nNumColumns = other.GetNumColumns();
	m_nNumRows = other.GetNumRows();
	m_pData = NULL;
	...
	if (bSuccess)
		// copy the pointer
		memcpy(m_pData, other.m_pData, sizeof(CComplex)*m_nNumColumns*m_nNumRows);
}
...
// 返回值：bool 型，初始化是否成功
//////////////////////////////////////////////////////////////////////
bool CComplexMatrix::Init(int nRows, int nCols)
{
	if (m_pData)
	{
		delete[] m_pData;
	...
	// 分配内存
	m_pData = new CComplex[nSize];

	if (m_pData == NULL)
		return false;					// 内存分配失败
	if (IsBadReadPtr(m_pData, sizeof(CComplex)* nSize))
		return false;
...
// 返回值：bool 型，初始化是否成功
//////////////////////////////////////////////////////////////////////
bool CComplexMatrix::MakeUnitMatrix(int nSize)
{
	if (!Init(nSize, nSize))
		return false;
...
// 返回值：bool 型，转换是否成功
//////////////////////////////////////////////////////////////////////
bool CComplexMatrix::FromString(string s, const string& sDelim /*= " "*/, bool bLineBreak /*= true*/)
{
	if (s.empty())
		return false;
...
// 返回值：string 型，转换得到的字符串
//////////////////////////////////////////////////////////////////////
string CComplexMatrix::ToString(const string& sDelim /*= " "*/, bool bLineBreak /*= true*/) const
{
	string s = "";
	...
	for (int i = 0; i<m_nNumRows; ++i)
	{
		for (int j = 0; j<m_nNumColumns; ++j)
		{
			string ss;
			ostringstream ostr;
			ostr << GetElement(i, j).ToString(false);
			ss = ostr.str();
			s += ss;
...
// 返回值：string 型，转换得到的字符串
//////////////////////////////////////////////////////////////////////
string CComplexMatrix::RowToString(int nRow, const string& sDelim /*= " "*/) const
{
	string s = "";
	...
	for (int j = 0; j<m_nNumColumns; ++j)
	{
		string ss;
		ostringstream ostr;
		ostr << GetElement(nRow, j).ToString(false);
		ss = ostr.str();
		s += ss;
		if (j != m_nNumColumns - 1)
			s += sDelim;
...
// 返回值：string 型，转换得到的字符串
//////////////////////////////////////////////////////////////////////
string CComplexMatrix::ColToString(int nCol, const string& sDelim /*= " "*/) const
{
	string s = "";
	...
	for (int i = 0; i<m_nNumRows; ++i)
	{
		string ss;
		ostringstream ostr;
		ostr << GetElement(i, nCol).ToString(false);
		ss = ostr.str();
		s += ss;
		if (i != m_nNumRows - 1)
			s += sDelim;
...
// 返回值：无
//////////////////////////////////////////////////////////////////////
void CComplexMatrix::SetData(CComplex value[])
{
	// empty the memory
	memset(m_pData, 0, sizeof(CComplex)* m_nNumColumns*m_nNumRows);
	// copy data
	memcpy(m_pData, value, sizeof(CComplex)*m_nNumColumns*m_nNumRows);
}
...
// 返回值：bool 型，说明设置是否成功
//////////////////////////////////////////////////////////////////////
bool CComplexMatrix::SetElement(int nRow, int nCol, CComplex& pValue)
{
	if (nCol < 0 || nCol >= m_nNumColumns || nRow < 0 || nRow >= m_nNumRows)
		return false;						// array bounds error
	if (m_pData == NULL)
	...
	m_pData[nCol + nRow * m_nNumColumns].SetData(pValue.GetReal(), pValue.GetImag());
...
// 返回值：double 型，指定元素的值
//////////////////////////////////////////////////////////////////////
CComplex CComplexMatrix::GetElement(int nRow, int nCol) const
{
	//	ASSERT(nCol >= 0 && nCol < m_nNumColumns && nRow >= 0 && nRow < m_nNumRows); // array bounds error
	//	ASSERT(m_pData);							// bad pointer error
	if (m_pData && nCol >= 0 && nCol < m_nNumColumns && nRow >= 0 && nRow < m_nNumRows)
...
// 返回值：int 型，矩阵的列数
//////////////////////////////////////////////////////////////////////
int	CComplexMatrix::GetNumColumns() const
{
	return m_nNumColumns;
}
...
// 返回值：int 型，矩阵的行数
//////////////////////////////////////////////////////////////////////
int	CComplexMatrix::GetNumRows() const
{
	return m_nNumRows;
}
...
// 返回值：double型指针，指向矩阵各元素的数据缓冲区
//////////////////////////////////////////////////////////////////////
CComplex* CComplexMatrix::GetData() const
{
	return m_pData;
}
...
// 返回值：int 型，向量中元素的个数，即矩阵的列数
//////////////////////////////////////////////////////////////////////
int CComplexMatrix::GetRowVector(int nRow, CComplex* pVector) const
{
	if (pVector == NULL)
		delete pVector;

	pVector = new CComplex[m_nNumColumns];
	//	ASSERT(pVector != NULL);
...
// 返回值：int 型，向量中元素的个数，即矩阵的行数
//////////////////////////////////////////////////////////////////////
int CComplexMatrix::GetColVector(int nCol, CComplex* pVector) const
{
	if (pVector == NULL)
		delete pVector;

	pVector = new CComplex[m_nNumRows];
	//	ASSERT(pVector != NULL);
...
// 返回值：CMatrix型的引用，所引用的矩阵与other相等
//////////////////////////////////////////////////////////////////////
CComplexMatrix& CComplexMatrix::operator=(const CComplexMatrix& other)
{
	if (&other != this)
	{
		bool bSuccess = Init(other.GetNumRows(), other.GetNumColumns());
...
// 返回值：bool 型，两个矩阵相等则为true，否则为false
//////////////////////////////////////////////////////////////////////
bool CComplexMatrix::operator==(const CComplexMatrix& other) const
{
	// 首先检查行列数是否相等
	if (m_nNumColumns != other.GetNumColumns() || m_nNumRows != other.GetNumRows())
		return false;
...
// 返回值：CMatrix型，指定矩阵与other相加之和
//////////////////////////////////////////////////////////////////////
CComplexMatrix	CComplexMatrix::operator+(const CComplexMatrix& other) const
{
	// 首先检查行列数是否相等
	//	ASSERT(m_nNumColumns == other.GetNumColumns() && m_nNumRows == other.GetNumRows());

	// 构造结果矩阵
	CComplexMatrix	result(*this);		// 拷贝构造
	if (m_nNumColumns != other.GetNumColumns() || m_nNumRows != other.GetNumRows())
		return result;
	// 矩阵加法
	for (int i = 0; i < m_nNumRows; ++i)
...
CComplexMatrix	CComplexMatrix::operator-(const CComplexMatrix& other) const
{
	...
	// 构造目标矩阵
	CComplexMatrix	result(*this);		// copy ourselves
	if (m_nNumColumns != other.GetNumColumns() || m_nNumRows != other.GetNumRows())
		return result;
	// 进行减法操作
	for (int i = 0; i < m_nNumRows; ++i)
...
// 返回值：CMatrix型，指定矩阵与value相乘之积
//////////////////////////////////////////////////////////////////////
CComplexMatrix	CComplexMatrix::operator*(double value) const
{
	// 构造目标矩阵
	CComplexMatrix	result(*this);		// copy ourselves
	// 进行数乘
...
CComplexMatrix	CComplexMatrix::operator*(const CComplexMatrix& other) const
{
	...
	// construct the object we are going to return
	CComplexMatrix	result(m_nNumRows, other.GetNumColumns());
	if (m_nNumColumns != other.GetNumRows())
		return result;
	...
	for (int i = 0; i < result.GetNumRows(); ++i)
	{
		for (int j = 0; j < other.GetNumColumns(); ++j)
		{
			...
			for (int k = 0; k < m_nNumColumns; ++k)
			{
				value = value + GetElement(i, k) * other.GetElement(k, j);
			}
...
// 返回值：CMatrix型，指定矩阵转置矩阵
//////////////////////////////////////////////////////////////////////
CComplexMatrix CComplexMatrix::Transpose() const
{
	// 构造目标矩阵
	CComplexMatrix	Trans(m_nNumColumns, m_nNumRows);
	...
	for (int i = 0; i < m_nNumRows; ++i)
	{
		for (int j = 0; j < m_nNumColumns; ++j)
		{
			CComplex complex = GetElement(i, j);
			complex.SetImag(-1 * complex.GetImag());
			Trans.SetElement(j, i, complex);
		}
	}
