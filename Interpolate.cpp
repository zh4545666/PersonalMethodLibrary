...
// 4. const string& sDelim - 数字之间的分隔符，默认为空格
//
// 返回值：int 型，转换成功的结点的个数
//////////////////////////////////////////////////////////////////////
int CInterpolate::GetNodesFromString(string s, int n, double dblNodes[], const string& sDelim /*= " "*/)
{
	// 将结点值初始化为0
...
//
// 参数：
// 1. double x[] - 一维数组，长度为n，存放给定的n个x坐标序列
// 2. double y[] - 一维数组，长度为n，存放给定的n个y坐标序列
// 3. double n - 数组长度
// 4. double ddy1 - 第一点二阶导数
// 5. double ddy2 - 最末点二阶导数
// 6. double t[] - 一维数组，长度为n，存放给定的m个插值点的x坐标序列
// 7. double m - 插值点个数
// 8. double z[] - 一维数组，长度为n，存放给定的m个差值点的y坐标序列
//                 z(i,j) = f(x(i), y(j)), i=0,1,...,n-1, j=0,1,...,m-1
//
// 返回值：double 型，0值
//////////////////////////////////////////////////////////////////////
double CInterpolate::Spline(double x[],double y[],int n,double ddy1, double ddyn,double t[],int m,double z[])
{
	int i, j;
...
// 1. double *x - 一维数组，长度为n，存放给定的n个x坐标序列
// 2. double *y - 一维数组，长度为n，存放给定的n个y坐标序列
// 3. double xx - 插值点的x坐标序列
// 4. double n - 数组长度
//
// 返回值：double 型，插值点的y坐标序列
//////////////////////////////////////////////////////////////////////
double CInterpolate::Lagrange(double *x, double *y, double xx, int n)
{
	int i, j;
